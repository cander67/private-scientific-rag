...
User testing of the review branch also found that Chat Workspace context inspection exposed retrieval previews as the assembled model context. The inspector should keep compact preview fields for retrieval review, but the assembled message must contain the full stored chunk text sent to the local model.
...
- Hydrate Chat Workspace context inspection with full chunk text for model prompt assembly while preserving snippet/text-preview fields as the collapsed retrieved-context display. Each retrieved context entry can expand or collapse the full context next to the existing source-navigation action.
...
11. As a researcher, I want retrieved context entries to show compact previews by default with an expand/collapse control, so that I can audit full chunks without losing scanability.
12. As a maintainer, I want assembled Chat Workspace messages to include the full stored chunk text sent to the model, so that context inspection accurately reflects the LLM payload.
...
- Chat context inspection full-chunk hydration for model assembly, draft preview, and persisted answer inspection.
- Retrieved-context expand/collapse controls in Chat Workspace inspection while retaining preview fields.
- Tests for settings bottom save, stale repair calls, state refresh, and chat rename.
- Tests for full-context prompt assembly and retrieved-context expansion affordances.
...
- Automatically shrinking or summarizing retrieved chunks to fit a model context window.
...
- Chat context preview and persisted answer inspection keep `snippet`/`text_preview` as compact preview fields and expose full chunk text separately for expansion.
- Chat prompt assembly uses full stored chunk text for retrieved entries, and the inspector's assembled messages match the payload sent to the model.
- Retrieved context entries in Chat Workspace inspection include an expand/collapse control next to source navigation when full context is available.
...
- Keep retrieval result preview fields intact for Search Lab and compact inspection display. Chat Workspace may hydrate a full-context field after retrieval, scoped to context preview and persisted chat inspection payloads.
- Treat the inspector's assembled messages as the audit source for exactly what is sent to the local model; do not show a shortened model payload unless a future explicit context-window policy is introduced.
...
- Chat context tests should prove full stored chunks reach assembled messages while snippets/text previews remain compact review fields.
- Frontend contract tests should verify retrieved context entries expose an expand/collapse full-context affordance beside source navigation.

...
- In progress: PRD28 document manager preview and batch actions on branch `doc-manager-upgrades`; Phases 1 through 3 add row-click metadata preview from `DocumentRead.current_version`, backend batch reprocess/OCR/delete routes with per-document outcomes, and Document Manager checkbox multi-select with selected-action controls while Source Viewer remains the explicit chunks/pages/provenance inspection path.

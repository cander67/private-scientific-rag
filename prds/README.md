...
- In progress: PRD28 document manager preview and batch actions on branch `doc-manager-upgrades`; Phase 1 adds row-click metadata preview from `DocumentRead.current_version`, and Phase 2 adds backend batch reprocess/OCR/delete routes with per-document outcome reporting while Source Viewer remains the explicit chunks/pages/provenance inspection path.
...
- Document Manager batch scope: PRD28 owns row-click metadata preview and selected-document batch actions for reprocess, OCR, and delete as a follow-up to the completed PRD3 Document Manager. Phase 1 row preview and Phase 2 backend batch routes are implemented on `doc-manager-upgrades`; selected rows preview summary metadata, and the API now returns explicit selected/full-repository reprocess, OCR, and delete batch outcomes for the later multi-select UI phase.

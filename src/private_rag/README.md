...
- `search/`: SQLite FTS5 schema management, sparse index rebuilds, query normalization, field weighting, and result shaping.
...
- `POST /repositories/{repository_id}/full-text/search`: searches indexed chunks and returns BM25 score, snippet, matched fields, document/chunk metadata, and citation-ready provenance.
...
- PRD4 full-text search is in progress: the initial sparse index rebuild and search API are available.

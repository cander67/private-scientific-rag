...
- PRD13 parser selection, OCR, and page-image text recovery is ready for final review, not complete until user acceptance: upload/reprocess honor repository parser settings for parser routing, reprocess creates versioned refreshed output from stored sources, document metadata reports current/stale parser-chunk status, index rebuilds block stale parser/chunk fingerprints, local OCR recovery stores inspectable page artifacts/chunks, RapidOCR can be selected as an optional fallback, OCR quality controls are repository settings, and parser/OCR route metadata records warnings and fallback decisions.

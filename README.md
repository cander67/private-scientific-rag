...
PRD1 through PRD9, PRD15, and PRD19 through PRD23, PRD26, and PRD27 are complete and closed. PRD13 Parser Selection, OCR, and Page-Image Text Recovery is ready for final review after parser routing, reprocess, stale-index freshness gates, local OCR recovery, RapidOCR fallback, fixed-versus-recursive chunking remediation, parser-label clarity remediation, and documentation preparation; it is not marked complete until user acceptance and the PRD29 manual acceptance pass. PRD25 Chat Context Inspection is ready for final review after draft context preview, persisted assistant-answer inspection, Chat Workspace inspector UI, source navigation, and deterministic backend/frontend coverage; it is not marked complete until user acceptance. PRD29 proposes the manual acceptance-test pass needed to exercise PRD13 with real local OCR dependencies and realistic documents before closing it. PRD30 drafts the next chunking direction: token-aware defaults, model-specific SentenceTransformers tokenizers, explicit Ollama tokenizer registry/fallback behavior, and tokenizer metadata for source inspection/export. The project now has the local app foundation, repository settings/reproducibility, document ingestion/source inspection, inspectable SQLite FTS5 search, dense vector search with additional local embedding models, hybrid Reciprocal Rank Fusion, selectable reranking, retrieval evaluation, local Ollama-backed retrieval-augmented chat with citations and expanded local chat model registry/readiness support, chat context inspection for normal chat transparency, a Prompt Sandbox for prompt/retrieval/model comparisons, portable export/recreate workflows for moving repositories across supported local hosts, guarded repository administration/reset workflows, a repository-scoped Settings / Models manager with catalog-backed model, parser, OCR, and retrieval-default guardrails, and a dashboard home surface for repository status and workflow navigation.

Later PRDs include structured table extraction (PRD14), bulk patent downloads/raw patent-data feeds (PRD12), clearer chunk-level versus document-level search labels (PRD17), token-aware chunking (PRD30), and immutable document storage. Immutable multi-index embedding comparison remains PRD16 scope.
...
- Document upload, PDF parser fallback chain, page-thumbnail generation, fixed or recursive parsing/chunking, source inspection, reprocess, explicit OCR recovery, OCR fallback, and delete API for PDF, TXT, Markdown, and ANN files.

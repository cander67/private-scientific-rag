...
| RapidOCR | Optional fallback provider | `uv run python -c "from rapidocr import RapidOCR; RapidOCR()"` after `uv sync --all-extras --dev` | RapidOCR initializes and fallback behavior is exercised or reported as not triggered because OCR quality is sufficient. | Import succeeds but engine initialization or fallback selection fails unexpectedly. | RapidOCR cannot initialize on this workstation; record RapidOCR fallback as skipped with the warning shown by the app. |

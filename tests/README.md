...
PRD9 export/import/recreate coverage has started with the portable bundle contract and bundle validation. Default tests cover deterministic source bundle paths and SHA-256 hashing, manifest schema validation, ZIP structure, default source inclusion, chat/retrieval/chunk/citation payload export, opt-in sandbox payload export, malformed/unsupported bundle rejection, missing payload detection, source hash mismatches, external source mapping failures, renamed-file warnings, missing model reports, parser fingerprints, and exported count summaries.

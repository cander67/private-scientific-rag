...
PRD13 parser-selection coverage is ready for final review and remains deterministic by default. Phase 1 default tests cover `Auto` preserving the PRD3 parser chain, explicit parser routing for selected structured parsers, dependency-missing fallback warnings, repository parser settings reaching upload, and parser fingerprint metadata recorded on document versions and chunks. Phase 2 default tests cover reprocess from stored sources, new-version creation while preserving prior inspectable chunks/artifacts, missing-source and parser-failure recovery, unchanged and changed parser fingerprints, document-level stale reprocess status after parser/chunking changes, and Source Viewer/Document Manager contract coverage for reprocess status/actions. Phase 3 default tests cover stale parser/chunk conflicts for full-text and vector rebuilds, clean rebuild after reprocess, current-version-only indexing after historical chunks are preserved, dashboard current-chunk counts, Chat Workspace rebuild error propagation, and existing export/recreate parser fingerprint validation. Phase 4 default tests cover deterministic page OCR routing for born-digital/scanned/mixed pages, route thresholds, local renderer handoff/provenance, normalized OCR page results, dependency-missing warning paths, parser metadata stamping, and Source Viewer OCR state labels. Phase 5 default tests cover the explicit document OCR action with synthetic provider output, stored OCR page artifacts, OCR-derived chunk metadata/provenance, missing local provider recovery, API action coverage, and Source Viewer OCR text/chunk labels. Phase 6 default tests cover optional RapidOCR fallback selection, low-quality fallback based on confidence/text signals, missing fallback dependency warnings, disabled fallback behavior, OCR settings validation, settings impact analysis, warning propagation, and Settings / Models OCR controls. User-testing remediation tests cover fixed versus recursive chunking behavior, chunking metadata, and Source Viewer parser-name/version labeling. Golden-corpus and live OCR dependency checks stay opt-in and are not default CI prerequisites.
...
PRD25 chat context inspection coverage is ready for final review and remains deterministic by default. Unit and integration tests cover draft context preview through the same prompt/retrieval assembly path as normal chat, no draft-message persistence, explicit empty-context status, persisted assistant-message context snapshots, retrieval-run linkage, `context_inspection_available`, and unavailable-state behavior for older assistant messages without snapshots. Frontend contract tests cover draft and persisted `Inspect context` controls, inspector sections for model/prompt/retrieval settings/history/context entries/assembled messages, empty-state messaging, and Source Viewer navigation for retrieved context entries. PRD32 user-testing remediation adds coverage that retrieved context keeps compact `snippet`/`text_preview` fields while full stored chunk text is hydrated into assembled messages and exposed through expand/collapse controls. Live Ollama checks remain opt-in because default coverage mocks the local LLM boundary.
...
PRD23 Settings model catalog coverage is complete and keeps catalog loading deterministic and separate from runtime probes. Unit and integration tests cover the repository-scoped model catalog response, known embedding dimensions and supported distances, known chat and reranker choices, and the not-checked runtime detection section that avoids Ollama calls or local model loading by default. Remediation coverage verifies Ollama embedding readiness through the configured runtime, `/api/embed` plus legacy `/api/embeddings` fallback, warm-up `keep_alive` payloads, missing-model guidance, timeout/load-in-progress/load-failed messaging, installed-but-not-loaded classification when `/api/tags` lists the model, startup-friendly timeouts, deterministic retrieval-history retention, and SentenceTransformers accelerator-to-CPU fallback behavior. Frontend contract tests cover Settings / Models catalog loading, known embedding dropdowns, derived read-only dimensions, disabled incompatible distances, catalog-backed chat/reranker choices, disabled reranking, detected-versus-known labels, Qdrant collection explanation/state/workflow links, advanced custom model paths, and Chat Workspace visibility for configured retrieval embedding and latest vector-index model details. Live Ollama, Qdrant, SentenceTransformers cache, cross-encoder, and GPU checks remain opt-in because they depend on local host/runtime state.

PRD27 Retrieval Defaults coverage is complete and keeps default resolution deterministic. Unit tests cover shared retrieval defaults validation, old chat/sandbox settings normalization, precedence/source tracking, and metadata boost `off` scoring. Integration tests cover repository retrieval defaults, settings impact, export/recreate compatibility through settings payloads, new chat-session inheritance, complete chat retrieval requests, and sandbox effective-setting snapshots. Frontend contract tests cover Settings / Models retrieval-default controls, Search Lab effective-settings copy/promote actions, metadata boost `off` controls and score breakdowns, Chat Workspace advanced retrieval controls, and Prompt Sandbox effective-setting display.

PRD30 Token-Aware Chunking coverage keeps default CI deterministic. Unit tests cover tokenizer resolution for SentenceTransformers, manual HuggingFace/SentenceTransformers tokenizers, mocked Ollama runtime tokenization, unsupported/malformed Ollama tokenizer fallback, known Ollama registry metadata, custom Ollama fallback metadata, unavailable exact-tokenizer fallback, recursive token coalescing, oversized segment token splitting, fixed token windows, token-window source spans, parser/chunk freshness, and calibrated 512-token/64-token-overlap defaults. Integration tests cover upload/reprocess token metadata, model-catalog tokenizer metadata, export bundle tokenizer metadata, recreate validation warnings for fallback and mismatched tokenizer metadata, and default repository settings. Frontend contract tests cover token labels, manual tokenizer controls, resolved tokenizer copy, Source Viewer chunk token/tokenizer metadata, export summaries, and recreate issue labels. Live SentenceTransformers cache and Ollama runtime checks remain opt-in because they depend on local model availability.

PRD32 Workflow Friction Remediation coverage is ready for final review and remains deterministic by default. Frontend contract tests cover bottom Settings / Models save/cancel controls sharing the top handlers/state, Search Lab stale parser/chunk repair CTAs using the PRD28 full-repository batch reprocess contract, Chat Workspace stale parser/chunk repair CTAs across readiness/rebuild/context-preview/ask failure states, post-reprocess refresh markers, manual rebuild boundaries, chat rename UI/default-title behavior, and retrieved-context expand/collapse controls. Backend integration tests cover chat-session rename success, non-empty title validation, missing/wrong-repository behavior, distinct default chat titles, preservation of messages/retrieval settings/model/prompt ID/repository ownership, and full stored chunk text reaching Chat Workspace assembled model messages while preview fields remain compact. The full review gate also runs deterministic backend tests, frontend tests, frontend build, formatting, linting, and typing; live Qdrant/Ollama/model checks remain opt-in.

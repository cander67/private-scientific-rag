...
PRD30 Token-Aware Chunking coverage is complete and keeps default CI deterministic. Unit tests cover tokenizer resolution for SentenceTransformers, known Ollama registry metadata, custom Ollama fallback metadata, recursive token coalescing, oversized segment token splitting, fixed token windows, token-window source spans, parser/chunk freshness, and calibrated 512-token/64-token-overlap defaults. Integration tests cover upload/reprocess token metadata, export bundle tokenizer metadata, recreate validation warnings for fallback and mismatched tokenizer metadata, and default repository settings. Frontend contract tests cover token labels, tokenizer strategy copy, Source Viewer chunk token/tokenizer metadata, export summaries, and recreate issue labels. Live SentenceTransformers cache and Ollama runtime checks remain opt-in because they depend on local model availability.

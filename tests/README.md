...
PRD13 parser-selection coverage is in progress. Phase 1 default tests cover `Auto` preserving the PRD3 parser chain, explicit parser routing for selected structured parsers, dependency-missing fallback warnings, repository parser settings reaching upload, and parser fingerprint metadata recorded on document versions and chunks. Phase 2 default tests cover reprocess from stored sources, new-version creation while preserving prior inspectable chunks/artifacts, missing-source and parser-failure recovery, unchanged and changed parser fingerprints, document-level stale reprocess status after parser/chunking changes, and Source Viewer/Document Manager contract coverage for reprocess status/actions. Phase 3 default tests cover stale parser/chunk conflicts for full-text and vector rebuilds, clean rebuild after reprocess, current-version-only indexing after historical chunks are preserved, dashboard current-chunk counts, Chat Workspace rebuild error propagation, and existing export/recreate parser fingerprint validation. Phase 4 default tests cover deterministic page OCR routing for born-digital/scanned/mixed pages, route thresholds, local renderer handoff/provenance, normalized OCR page results, dependency-missing warning paths, parser metadata stamping, and Source Viewer OCR state labels. Phase 5 default tests cover the explicit document OCR action with synthetic provider output, stored OCR page artifacts, OCR-derived chunk metadata/provenance, missing local provider recovery, API action coverage, and Source Viewer OCR text/chunk labels. Golden-corpus OCR checks remain opt-in PRD13 work.
